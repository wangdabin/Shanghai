#!/bin/bash
###################################################################
#*名称 --%@NAME:P_ODS_BUS_EXPBILL_RENT_D
#*功能描述 --%@COMMENT: 营帐平台实时账单（SQLLDR导入）
#*执行周期 --%@PERIOD:日
...
#*修改记录 --%@MODIFY: 暂无
#*所属于实体--%@ENTITY: 
#*来源表 --%@FROM:SRC.EXPBILL_RENT
#*目标表 --%@TO:ODS.ODS_BUS_EXPBILL_RENT_D
####################################################################
...
table_name='ODS_BUS_EXPBILL_RENT_D'
