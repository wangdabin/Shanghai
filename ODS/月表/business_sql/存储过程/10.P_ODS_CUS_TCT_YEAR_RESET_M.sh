#!/bin/bash
###################################################################
#*名称 --%@NAME:P_ODS_CUS_TCT_YEAR_RESET_M
#*功能描述 --%@COMMENT:作废积分月表(BSS为全量数据,通过OPERATE_DATE区分当月)
#*执行周期 --%@PERIOD:月
...
#*层次---%@LEVEL:ODS层 
#*主题域---%@MASTER_FIELD:客户域
#*备注 --%@REMARK: 重建数据仓库批量创建过程
#*修改记录 --%@MODIFY: 暂无
#*所属于实体--%@ENTITY: 
#*来源表 --%@FROM:SRC.TCT_CREDIT_YEAR_RESET
#*目标表 --%@TO:ODS.ODS_CUS_TCT_YEAR_RESET_M
####################################################################
...
table_name='ODS_CUS_TCT_YEAR_RESET_M'
