#!/bin/bash
###################################################################
#*名称 --%@NAME:电子渠道统计处用到该表SRC.RES_NUMBER_ORDER
#*功能描述 --%@COMMENT:电子渠道统计处用到该表SRC.RES_NUMBER_ORDER
#*执行周期 --%@PERIOD:月
...
#*创建人 --%@CREATOR:陶亮
#*迁移人 --%@MOVER:曹丙瑞
#*创建时间 --%@CREATED_TIME:2012-02-01
#*层次---%@LEVEL:ODS层 
#*主题域---%@MASTER_FIELD:资源域
#*备注 --%@REMARK: 电子渠道统计处用到该表SRC.RES_NUMBER_ORDER
#*修改记录 --%@MODIFY: 暂无
#*所属于实体--%@ENTITY: 
#*来源表 --%@FROM:ODS.ODS_RES_NUMBER_ORDER_D
#*目标表 --%@TO:ODS.ODS_RES_NUMBER_ORDER_M
####################################################################
...
table_name='ODS_RES_NUMBER_ORDER_M'
...
v_sql=`sh $business_sql_directory$v_procname'_SQL.sh' $v_month`
...
v_sql="select count(1) coun from $v_pkg.$table_name where month_part = '$v_month'"
